...
plot <- qplot(MDS1, MDS2, data=NMDS, main = "2017 Garlic Mustard", color=Treatment, alpha=0.5, size = 1) + guides(size = "none") + guides(alpha = 'none')
plot 
...
otu_rel_DOD <- read.csv("DOD2015_by_treatment.csv", header = T, sep = ",")
View(otu_rel_DOD)
dim(otu_rel_DOD) ####64, 4225
comm.num.DOD <- otu_rel_DOD[ ,2:4225]
View(comm.num.DOD)
set.seed(200)
library(vegan)
otu.nms_1 <- metaMDS(comm.num.DOD, distance = "bray", trymax = 2000, autotransform = F)
...
plot <- qplot(MDS1, MDS2, data=NMDS, main = "2015 Garlic Mustard", color=Treatment, alpha=0.5, size = 1) + guides(size = "none") + guides(alpha = 'none')
plot 
